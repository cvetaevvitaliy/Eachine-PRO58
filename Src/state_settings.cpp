...
#endif
static void audioSettingMenuHandler(Ui::SettingsMenuItem* item);
...
#endif
  const char* audioValue = (EepromSettings.audioEnabled ? "ON" : "OFF");
  this->menu.addItem("Audio", audioSettingMenuHandler, audioValue);
...
static void audioSettingMenuHandler(Ui::SettingsMenuItem* item){
      if (EepromSettings.audioEnabled)
        EepromSettings.audioEnabled = 0;
      else
        EepromSettings.audioEnabled = 1;

      EepromSettings.markDirty();

      const char* audioValue = (EepromSettings.audioEnabled ? "ON" : "OFF");
      item->value = audioValue;
}
